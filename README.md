...
在调试服务端时，服主通常需要频繁通过重命名文件后缀（如 `.jar` ↔ `.jar.disabled`）来排查插件冲突或临时停用插件。PlugLever 通过终端交互界面接管这一过程，提供单选切换、批量开关、实时搜索、元数据读取、SpigotMC 资源更新自动检测与安全重命名等功能，无需在文件管理器或终端命令间来回切换
...
- **SpigotMC 资源更新自动检测（静默无感）**：
  - **自动识别**：自动解析 `plugin.yml` / `paper-plugin.yml` 中的 `website` 网址，智能提取 SpigotMC 资源数字 ID（如 `spigotmc.org/resources/essentialsx.9089/` ➜ `9089`）。
  - **后台异步**：启动与手动刷新时，通过后台 Goroutine 并发发起异步请求（官方 Update API），界面秒开零卡顿。
  - **醒目标识**：若官方存在更新版本，列表中以高亮角标直观提示（如 `[v1.7.2 ➜ v1.7.3]`），并在插件详情浮层中同步标明 `(发现新版本！) `。
  - **零干扰容错**：未配置主页、非 Spigot 链接、网络超时或 API 不可达时，静默跳过，绝无弹窗或报错打扰。
- **版本号完全原样展示**：取消版本号长度截断，根据当前插件动态计算列宽，无论多么冗长的版本（如 `-SNAPSHOT`、构建号、git hash）均 100% 完整显示且右侧文件大小严格垂直对齐。
...
- **插件详情模态框 (Inspector)**：快捷键直接弹出浮层，完整查看插件注册名、版本（含更新提示）、作者、强依赖与软依赖项、主页、描述、修改时间与绝对路径。
